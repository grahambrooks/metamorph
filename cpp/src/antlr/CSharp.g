...
@lexer::namespace { CodeR }
@parser::namespace { CodeR }
/*
@lexer::traits 
{
	class CSharpLexer; 
...
primary_expression
	...
	|	'new'
			(
				(object_creation_expression   ('.'|'->'|'[')) => object_creation_expression   primary_expression_part+ 		// new Foo(arg, arg).Member
...
class_declaration
	:	'class'  type_or_generic   class_base?   type_parameter_constraints_clauses?   class_body   ';'? -> ^(CLASS_DECL type_or_generic   class_base?   type_parameter_constraints_clauses?   class_body   ';'?)
	;
class_base:
...
local_variable_declaration
	:	local_variable_type   local_variable_declarators -> ^(VARIABLE_DECL local_variable_type   local_variable_declarators)
	;
...
local_variable_declarators
	:	local_variable_declarator (',' local_variable_declarator)* -> local_variable_declarator+
	;
...
@after		
{
    //	CommonTokenType* int_literal = new CommonTokenType(NUMBER, $dil.text);
//	CommonTokenType* dot = new CommonTokenType(DOT, ".");
//	CommonTokenType iden = new CommonTokenType(IDENTIFIER, $s.text);
	
//	emit(int_literal);
//	emit(dot);
//	emit(iden);
//    printf("\tFound GooBall " + $text);
}
	:
...
//			if (Processing.size() > 0 && Processing.top()) {
...
fragment UNDEF_TOKEN
	...
	{
		//if (MacroDefines[$define.getText()])
...
fragment PP_EQUALITY_EXPRESSION
	:	PP_UNARY_EXPRESSION   TS*   (('=='| '!=')   TS*   PP_UNARY_EXPRESSION
		{ 
		}
		TS* )*
	;
...
fragment PP_UNARY_EXPRESSION:
	...
	| '!'   TS*   PP_UNARY_EXPRESSION  { /*Returns.push(!Returns.pop());*/ }
	;

fragment PP_PRIMARY_EXPRESSION:
	IDENTIFIER	
	{ 
		//Returns.push(MacroDefines.containsKey($IDENTIFIER.getText()));
	}
	| '('   PP_EXPRESSION   ')'
	;
